...
class _StubAuth implements AuthService {
  /// The only call the tests make. Everything else throwing keeps a new
  /// dependency on the auth service from passing unnoticed.
  @override
...
void main() {
  ...
  group('chat asks for a sign-in rather than answering', () {
    test('the demo needs one', () async {
      ...
      // fixture. That fixture must not go back as though it were an
      // engine's answer.
    ...
    testWidgets('the composer keeps what was typed when the send is refused',
      ...
      // Clearing the bar on a send that never happened throws away the
      // person's text and leaves nothing to show for it.
      SharedPreferences.setMockInitialValues(<String, Object>{});
